...
func Launch(args Args) {
	...
	if args.Method1 == PORTFORWARD_SOCK_CONN &&
		...
		color.Error.Println(time.Now().Format("01-02|15:04:05"), "Error", "unknown forward method")
		return
	}
}
...
func ListenConn(proto uint8, addr1 string, addr2 string) {
	...
	color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "listen A point with sock1 ", addr1)
	...
	for {
		...
		color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "A point(link", count, ") ", sock1.RemoteAddr(), " is ready")
		// socket2 dial
		color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "dial B point with sock2 ", addr2)
		sock2, err := sockfoo2(addr2)
		if err != nil {
			sock1.Close()
			color.Error.Println(time.Now().Format("01-02|15:04:05"), "ERROR", err)
			continue
		}
		color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "B point(sock2) is ready")
...
func ListenListen(proto uint8, addr1 string, addr2 string) {
	...
	color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "listen B point with sock2", addr2)
	...
	for {
		...
		case sock2 = <-clientc2:
			...
			color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO", "B point(link", count, ") ", sock2.RemoteAddr(), " is ready")
		case <-time.After(120 * time.Second):
			// LogWarn("socket wait timeout, reset")
			color.Warn.Println(time.Now().Format("01-02|15:04:05"), "WARN", "socket wait timeout, reset")
			release(sock1, sock2)
			continue
		}
...
func ConnConn(proto uint8, addr1 string, addr2 string) {
	...
	for {
		...
		// socket1 dial
		color.Info.Println(time.Now().Format("01-02|15:04:05"), "INFO ", " dial A point with sock1", addr1)
		...
		color.Success.Println(time.Now().Format("01-02|15:04:05"), "INFO", "A point(sock1) is ready")
		...
		if err != nil {
			color.Error.Println(time.Now().Format("01-02|15:04:05"), "B point :", err)
			continue
		}
...
func ConnectSock(id int, sock1 Conn, sock2 Conn) {
	...
	go func() {
		...
		if err != nil {
			color.Error.Println(time.Now().Format("01-02|15:04:05"), "Error", "ConnectSock", id, "(A=>B):", err)
			// LogError("ConnectSock%d(A=>B): %s", id, err)
		} else {
			color.Info.Println(time.Now().Format("01-02|15:04:05"), "Info", "ConnectSock", id, "(A=>B) exited")
	...
	go func() {
		...
		if err != nil {
			...
			color.Error.Println(time.Now().Format("01-02|15:04:05"), "Error", "ConnectSock", id, "(B=>A):", err)
		} else {
			// LogInfo("ConnectSock%d(B=>A) exited", id)
			color.Info.Println(time.Now().Format("01-02|15:04:05"), "Info", "ConnectSock", id, "(B=>A) exited")
		}
		exit <- true
	}()
